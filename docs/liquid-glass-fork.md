...
- On macOS 26+, use a transparent AppKit panel, a light frosted outer background, and clear AppKit NSGlassEffectView surfaces. The native effect supplies its own optical edge and shadow; there are no additional painted strokes, bevels, or card shadows. A neutral tint supports white foreground content; the outer window keeps a light appearance. Older systems and Swift 6.1 builds retain NSPopover and system material backgrounds.
- Pair adjacent battery and network summaries into 64-point-high capsules while preserving configured section order and visibility. Audio and Bluetooth remain full width. Place the app name, settings, and quit controls in a subdued footer.
